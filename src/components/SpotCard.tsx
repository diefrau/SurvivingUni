...
const statusLabel: Record<Spot['status'], string> = {
  verified: '확인됨',
  needs_check: '확인 필요',
  closed: '운영 종료',
  unknown: '미확인',
  ...
  return (
    <Pressable onPress={onPress} style={styles.card}>
      <View style={styles.row}>
        ...
        <Text style={styles.badge}>{statusLabel[spot.status]}</Text>
      </View>
      <Text style={styles.meta}>{building?.name ?? '건물 확인 중'} · {spot.floor}</Text>
      <Text style={styles.tags}>{spot.tags.map((tag) => `#${tag}`).join(' ')}</Text>
      <Text style={styles.summary}>{spot.summary}</Text>
      <Text style={styles.verified}>최근 확인 {spot.lastVerifiedAt} · 확인 {spot.verificationCount}회</Text>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  badge: { color: colors.primary, backgroundColor: colors.primarySoft, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999, overflow: 'hidden', fontWeight: '800' },
  meta: { color: colors.muted, marginTop: 6, fontWeight: '700' },
  tags: { color: colors.primary, marginTop: 10, fontWeight: '700' },
  summary: { color: colors.text, marginTop: 10, lineHeight: 20 },
  verified: { color: colors.muted, marginTop: 10, fontSize: 12 },
});
